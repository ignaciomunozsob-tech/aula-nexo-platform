...
export default function CreatorPlanPage() {
  ...
  const plan = (data?.plan as string) || 'gratis';
  const comision = (data?.comision as number) ?? 10;
  const vence = data?.plan_vence as string | null;
          ...
          <>
            <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
              <div>
                ...
                <h2 className="text-3xl font-bold">{planLabel[plan] ?? plan}</h2>
                {vence && (
                  <p className="text-sm text-muted-foreground mt-2">
                    Vence el {new Date(vence).toLocaleDateString('es-CL')}
                  </p>
                )}
              </div>
              <div className="text-right">
                <p className="text-sm font-semibold text-muted-foreground mb-1">Comisión por venta</p>
                <div className="text-4xl font-bold" style={{ color: '#fcc70e' }}>{comision}%</div>
              </div>
            </div>
            ...
            <ul className="space-y-2 mb-6">
              {plan === 'gratis' && (
                <>
                  <li className="flex items-center gap-2 text-sm"><Check className="h-4 w-4" style={{ color: '#fcc70e' }} /> Hasta 2 cursos publicados</li>
                  <li className="flex items-center gap-2 text-sm"><Check className="h-4 w-4" style={{ color: '#fcc70e' }} /> Videos vía YouTube o Vimeo</li>
                  <li className="flex items-center gap-2 text-sm"><Check className="h-4 w-4" style={{ color: '#fcc70e' }} /> Archivos hasta 10MB</li>
                </>
              )}
              {plan === 'creador' && (
                <>
                  <li className="flex items-center gap-2 text-sm"><Check className="h-4 w-4" style={{ color: '#fcc70e' }} /> Cursos ilimitados</li>
                  <li className="flex items-center gap-2 text-sm"><Check className="h-4 w-4" style={{ color: '#fcc70e' }} /> Subida directa de videos</li>
                  <li className="flex items-center gap-2 text-sm"><Check className="h-4 w-4" style={{ color: '#fcc70e' }} /> Archivos ilimitados</li>
                </>
              )}
            </ul>
            ...
            <Button asChild>
              <Link to="/precios">
                ...
                {plan === 'gratis' ? 'Mejorar mi plan' : 'Ver todos los planes'}
              </Link>
            </Button>
          </>
        )}
